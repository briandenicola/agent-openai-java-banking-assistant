...
public class PortfolioAgent {
    ...
    private String ACCOUNT_AGENT_SYSTEM_MESSAGE = """
     you are a personal financial advisor who help the user to retrieve information about their bank portfolios.
     Use html list or table to display the portfolio information.
     Always use the below logged user details to retrieve portfolio info:
     %s
    ...
    public PortfolioAgent(OpenAIAsyncClient client, LoggedUserService loggedUserService,ToolsExecutionCache toolsExecutionCache, String modelId,String portfolioAPIUrl) {
        this.client = client;
        ...
        String portfoliosAPIYaml = null;
        try {
            portfoliosAPIYaml = EmbeddedResourceLoader.readFile("portfolio.yaml",
                    HistoryReportingAgent.class,
                    EmbeddedResourceLoader.ResourceLocation.CLASSPATH_ROOT);
        } catch (FileNotFoundException e) {
            throw new RuntimeException("Cannot find portfolio-history.yaml file in the classpath", e);
        }
        //Used to retrieve portfolio id. Transaction API requires portfolio id to retrieve transactions
        KernelPlugin openAPIImporterPortfolioPlugin = SemanticKernelOpenAPIImporter
                .builder()
                .withPluginName("PortfoliosPlugin")
                .withSchema(portfoliosAPIYaml)
                .withServer(portfolioAPIUrl)
                .build();


        kernel = Kernel.builder()
                .withAIService(ChatCompletionService.class, chat)
                .withPlugin(openAPIImporterPortfolioPlugin)
     ...
     public void run (ChatHistory userChatHistory, AgentContext agentContext){
         LOGGER.info("======== Portfolio Agent: Starting ========");
